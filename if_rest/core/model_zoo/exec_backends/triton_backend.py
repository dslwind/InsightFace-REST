...
from if_rest.core.model_zoo.exec_backends.abstract import (
    AbstractArcFace, AbstractDetectorInfer)
...
class Arcface(AbstractArcFace):
    ...
    def prepare(self, **kwargs):
        ...
        logger.debug(f"Model metadata: {self.model_name} {self.model_version}")
        try:
        ...
        self.triton_client.register_cuda_shared_memory(
            self.out_handle_name,
            cudashm.get_raw_handle(self.out_handle),
            0,
            self.out_bytesize,
        )
    ...
    def get_embedding(self, face_img):
        ...
        inputs.append(
            grpcclient.InferInput(
                self.input_name, [blob.shape[0], self.c, self.h, self.w], "FP32"
            )
        )
...
class DetectorInfer(AbstractDetectorInfer):
    ...
    def run(self, input):
        ...
        inputs.append(
            grpcclient.InferInput(
                self.input_name, [1, self.c, self.h, self.w], self.dtype
            )
        )
