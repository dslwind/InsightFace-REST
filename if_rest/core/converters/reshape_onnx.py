...
def reshape(model, n: int = 1, h: int = 480, w: int = 640, mode="auto"):
    """
    Reshape ONNX model input and output for different image sizes. 
    Only applicable for MXNet Retinaface models and official Centerface models.
    ...
    if mode == "auto":
        ...
        if input_name == "input.1" and dyn_size is True:
            ...
            input_name == "input"
            and model.graph.input[0].type.tensor_type.shape.dim[2].dim_value == 112
        ):
            mode = "arcface"
        ...
        if (
            ...
            and len(model.graph.output[0].type.tensor_type.shape.dim) == 3
        ):
            mode = "yolov5-face"
...
def reshape_onnx_input(
    ...
    """
    Reshape ONNX file input and output for different image sizes. 
    Only applicable for MXNet Retinaface models and official Centerface models.
