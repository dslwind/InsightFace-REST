...
class ModelSettings(BaseSettings):
    ...
    # *** BUG FIX START ***
    # 将类型提示改为 Any，以绕过 Pydantic 的严格列表解析，让我们的验证器全权处理
    max_size: Any = Field([640, 640], description="输入图像的最大尺寸 [宽, 高]")

    @field_validator("max_size", mode="before")
    @classmethod
    def validate_max_size(cls, v: Any) -> List[int]:
        """将逗号分隔的字符串或列表转换为整数列表。"""
        if isinstance(v, str):
            if not v:
                return []
        ...
        if isinstance(v, list):
            return v
        raise TypeError("max_size 必须是逗号分隔的字符串或列表")
    ...
    triton_uri: Optional[str] = Field(
        None, description="Triton推理服务器的URI (e.g., 'localhost:8001')"
    )

    # 自定义类型验证器
    _validate_optional_models = field_validator(
        "ga_name", "mask_detector", "triton_uri", mode="before"
    )(empty_str_to_none)
...
class AppSettings(BaseSettings):
    ...
    root_images_dir: str = Field(
        "/images", description="用于从相对路径加载图片的根目录"
    )
